...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (DB::connection()->getDriverName() !== 'pgsql') {
            throw new LogicException('MIG-CK-LICENSES preflight requires PostgreSQL.');
        }

        $this->assertCandidateKey('license_inventory_entries', 'license_inventory_entry_candidate_key_org_id', ['organization_id', 'id'], false);
        $this->assertCandidateKey('license_assignments', 'license_assignment_candidate_key_org_id', ['organization_id', 'id'], false);
    }
    ...
    private function assertCandidateKey(string $table, string $name, array $columns, bool $allowKnownMissingLicenseProductColumn = false): void
    {
        ...
        foreach ($columns as $column) {
            if (! Schema::hasColumn($table, $column)) {
                ...
                throw new LogicException("MIG-CK-LICENSES preflight missing {$table}.{$column} for {$name}.");
            }
        }
        ...
        if ($duplicate !== null) {
            throw new LogicException("MIG-CK-LICENSES preflight found duplicate rows for {$name}; reviewed remediation is required before write-fence.");
        }
    }
