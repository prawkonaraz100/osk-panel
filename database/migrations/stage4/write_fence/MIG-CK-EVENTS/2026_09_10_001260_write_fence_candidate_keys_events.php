...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (DB::connection()->getDriverName() !== 'pgsql') {
            throw new LogicException('MIG-CK-EVENTS write-fence requires PostgreSQL.');
        }

        $this->addUniqueConstraint('audit_logs', 'audit_log_candidate_key_org_id', ['organization_id', 'id']);
        $this->addUniqueConstraint('domain_events', 'domain_event_candidate_key_org_id', ['organization_id', 'id']);
    }
    ...
    private function addUniqueConstraint(string $table, string $name, array $columns): void
    {
        if (! Schema::hasTable($table) || ! Schema::hasColumns($table, $columns)) {
            throw new LogicException("MIG-CK-EVENTS write-fence prerequisite is missing for {$name}.");
        }
...
SQL,
        ...
        if ($postcondition === null) {
            throw new LogicException("MIG-CK-EVENTS write-fence postcondition failed for {$name}.");
        }
    }
