...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (DB::connection()->getDriverName() !== 'pgsql') {
            throw new LogicException('MIG-CK-FINANCE write-fence requires PostgreSQL.');
        }

        $this->addUniqueConstraint('student_charges', 'student_charge_candidate_key_org_id_student_currency', ['organization_id', 'id', 'student_id', 'currency']);
    }
    ...
    private function addUniqueConstraint(string $table, string $name, array $columns): void
    {
        if (! Schema::hasTable($table) || ! Schema::hasColumns($table, $columns)) {
            throw new LogicException("MIG-CK-FINANCE write-fence prerequisite is missing for {$name}.");
        }
...
SQL,
        ...
        if ($postcondition === null) {
            throw new LogicException("MIG-CK-FINANCE write-fence postcondition failed for {$name}.");
        }
    }
