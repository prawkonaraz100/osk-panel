...
{
    private const TABLE = 'organization_commerce_order_sequences';
    ...
    public function up(): void
    {
        ...
        if (DB::connection()->getDriverName() !== 'pgsql') {
            throw new LogicException('Commerce order sequence expand requires PostgreSQL.');
        }

        if (Schema::hasTable(self::TABLE)) {
            $this->assertExactTable();

            return;
        }

        DB::statement(<<<'SQL'
CREATE TABLE organization_commerce_order_sequences (
    organization_id uuid NOT NULL,
    next_order_sequence bigint NOT NULL,
    CONSTRAINT organization_commerce_order_sequences_pkey PRIMARY KEY (organization_id),
    CONSTRAINT organization_commerce_order_sequences_next_order_sequence_gte_1 CHECK (next_order_sequence >= 1),
    CONSTRAINT organization_commerce_order_sequences_organization_id_fk
...
SQL);
    ...
    private function assertExactTable(): void
    {
...
SQL);
        ...
        if (count($columns) !== 2
            ...
            throw new LogicException('Existing organization_commerce_order_sequences has the wrong column definition.');
        }
...
SQL);
        ...
        if (count($constraints) !== 3) {
            throw new LogicException('Existing organization_commerce_order_sequences has unexpected constraints.');
        }
        ...
        if ($primary === null
            ...
            throw new LogicException('Commerce order sequence primary key definition mismatch.');
        }
        ...
        if ($check === null
            ...
            throw new LogicException('Commerce order sequence positivity check definition mismatch.');
        }
        ...
        if ($foreign === null
            ...
            throw new LogicException('Commerce order sequence organization foreign key definition mismatch.');
        }
    }
